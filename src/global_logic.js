...
module.exports.calcDamage = function (summedAttack, totalSkillCoeff, criticalRatio, enemyDefense, defenseDebuff, enemyResistance, additionalRatio, damageUP, limitValues) {
    // Damage calculation
    ...
    normalDamage = damage + overedDamage;

    // "Granted Damage Increase" is a correction after attenuation
    normalDamage *= Math.max(1.0, 1.0 + damageUP);
    normalDamage *= Math.max(0.0, Math.min(1.0, 1.0 - enemyResistance));
    
    const additionalDamage = normalDamage * Math.max(0, additionalRatio);

    return [normalDamage, additionalDamage];
};
...
module.exports.calcBasedOneSummon = function (summonind, prof, buff, totals) {
    ...
    for (var key in totals) {
        ...
        var additionalRatio = 0.01 * totals[key]["additionalDamage"] * totalSummon["zeus"];
        additionalRatio += totals[key]["additionalDamageBuff"];
        additionalRatio += buff["additionalDamage"];
        additionalRatio += prof.shiToAiNoSekai ? 0.30 : 0;
        if (this.sum(totals[key]["additionalDamageXA"]) > 0) {
            ...
            additionalRatio += taRate * taDamage; // additionalDamage On Triple Attack
            additionalRatio += (1.0 - taRate) * daRate * daDamage; // additionalDamage On Double Attack
            additionalRatio += (1.0 - taRate) * (1.0 - daRate) * saDamage; // additionalDamage On Single Attack
        }
        if (totals[key]['echoThirdHit'] > 0 && taRate > 0) {
            additionalRatio += totals[key]['echoThirdHit'] * taRate / 3
        }
        ...
        const [damageWithoutAdditional, additionalDamage] = module.exports.calcDamage(summedAttack, totalSkillCoeff, criticalRatio, prof.enemyDefense, prof.defenseDebuff, enemyResistance, additionalRatio, damageUP + damageUPOnlyNormalDamage, normalDamageLimitValues);
        let damage = damageWithoutAdditional + additionalDamage;

        // Use damage in case of no critical to correct skill expectation
        const [damageWithoutCriticalAndAdditional, additionalDamageWithoutCritical] = module.exports.calcDamage(summedAttack, totalSkillCoeff, 1.0, prof.enemyDefense, prof.defenseDebuff, enemyResistance, additionalRatio, damageUP + damageUPOnlyNormalDamage, normalDamageLimitValuesWithoutCritical);
        let damageWithoutCritical = damageWithoutCriticalAndAdditional + additionalDamageWithoutCritical;
        ...
        coeffs["additionalRatio"] = additionalRatio;
        ...
        res[key] = {
            ...
            damageWithoutAdditional: damageWithoutAdditional,
        ...
        for (var j = 0; j < oneresult.length; j++) {
            ...
            for (var key in onedata) {
                ...
                for (var k = 0; k <= 100; k++) {
                    ...
                    const [damageWithoutAdditional, additionalDamage] = module.exports.calcDamage(summedAttack, newTotalSkillCoeff, onedata[key].criticalRatio, prof.enemyDefense, prof.defenseDebuff, onedata[key].skilldata.enemyResistance, onedata[key].skilldata.additionalRatio, onedata[key].skilldata.damageUP + onedata[key].skilldata.damageUPOnlyNormalDamage, onedata[key].normalDamageLimitValues)
                    var newDamage = damageWithoutAdditional + additionalDamage;
            ...
            for (var j = 0; j < turndata.length; j++) {
                ...
                for (var key in onedata) {
                    if (turnBuff.buffs["全体バフ"][t - 1].turnType == "ougi" || turnBuff.buffs[key][t - 1].turnType == "ougi") {
                        ...
                        // Regular attack
                        const [damageWithoutAdditional, additionalDamage] = module.exports.calcDamage(onedata[key].displayAttack, onedata[key].totalSkillCoeff, onedata[key].criticalRatio, prof.enemyDefense, prof.defenseDebuff, onedata[key].skilldata.enemyResistance, onedata[key].skilldata.additionalRatio, onedata[key].skilldata.damageUP + onedata[key].skilldata.damageUPOnlyNormalDamage, onedata[key].normalDamageLimitValues);
                        var newDamage = damageWithoutAdditional + additionalDamage;
                        if (key == "Djeeta") {
